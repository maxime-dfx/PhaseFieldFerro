...
struct Element {
    // Capacite maximale fixee a 8 (pour le Q8)
    ...
    // Acces direct ultra-rapide
    int get_node_index(int i) const {
        assert(i < num_nodes && "Index hors limites pour cet element !");
        return node_indices[i];
    }

    // Pointeur brut et iterateurs standards (0 allocation)
    const int* data() const { return node_indices.data(); }
