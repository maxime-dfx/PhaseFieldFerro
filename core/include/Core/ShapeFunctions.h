...
public:
    ...
    // ---------- TRIANGLE T3 (nouveau) ----------
    // Coordonnees barycentriques (xi, eta), N1 = 1-xi-eta, N2 = xi, N3 = eta
    ...
    static std::array<std::array<double,3>,2> get_shape_function_gradients_tri() {
        // Gradients constants pour un T3 (CST)
        return {{
            { -1.0, 1.0, 0.0 },   // dN/dxi
            { -1.0, 0.0, 1.0 }    // dN/deta
        }};
    ...
    // ---------- Version generique quad (inchangee, renommee implicitement par usage) ----------
